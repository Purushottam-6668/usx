...
if userge.has_bot:
    ...
    async def start_bot(_, message: Message):
        ...
        if from_user.id in Config.OWNER_ID:
            ...
            start_msg = f"""
Hello Sir / Mam 👋 {from_user.fname},
Nice To Meet You !, I'm  <b>{bot_.fname}</b> A Bot.

        <b><i>Powered By📨</i> [📚TEAM LIBRARIAN™️](https://t.me/Team_Librarian)

My Master is : {owner_.flname}</b>
"""
            if Config.BOT_FORWARDS:
                start_msg += "<b>\n📌 NOTE:</b>\nHey <b>{from_user.fname}<b> ,You can 📨 <b>Send Message To My Respected Master💝 </b> Press The Buttons To Contact My Sweet<b>Master.</b>"
            contact_url = (
            ...
            btns = [
                [
                    InlineKeyboardButton("☎️Contact☎️", url=contact_url),
                    InlineKeyboardButton("🤔SAURCE😁", url=Config.UPSTREAM_REPO),
                ]
            ]
        try:
    ...
    async def add_to_grp(c_q: CallbackQuery):
        await c_q.answer()
        msg = "<b>🤖ADD ME TO YOUR GROUP🥺</b> \n\n📌 **Note:**\n<i>Admin Privilege Required ! I am not usless ok</i>"
